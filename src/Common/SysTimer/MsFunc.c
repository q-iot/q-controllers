...
#define T_Debug(x,y...) //Debug
#define PrintNowTime()  T_Debug("                                                 @ %d Ms\r",GetSysStartMs());

#define TFUNC_RCD_NUM 32//总记录数

#if 1
static volatile u32 gThisTimStart=0;//本次开始定时的时间

//定时器单位ms
#define TfTimSet(Val) {StartSysTimer(gTimingFuncTimer,Val);gThisTimStart=GetSysStartMs();}//设定定时器
#define TfTimCancle() {StopSysTimer(gTimingFuncTimer);gThisTimStart=0;}//取消定时器

s32 GetTfTimCount(void)
{
	if(gThisTimStart)
	{
		s32 Gap=GetSysStartMs()-gThisTimStart;
		return Gap;
	}
...
void MsFuncRcdDisp(void)
{
	...
	for(i=0;i<TFUNC_RCD_NUM;i++)
	{
		...
		if(gTfRcd[i].pStdCB) Debug("AddMsFunc%x[%u]:%u ,",gTfRcd[i].pStdCB,i,gTfRcd[i].IntParam);
		else Debug("AddMsFunc%x[%u]:%u ,",gTfRcd[i].pExpCB,i,gTfRcd[i].IntParam);
...
//TimCnt为1，会执行低优先级的next func
//如果Interval==0，pCallBack将视为pStdFunc，这点要注意
//当TimCnt==-1时，无限循环，Interval为循环时间
bool AddMsFunc(u32 TimCnt,u32 Interval,pExpFunc pCallBack,int IntParam,void *pParam)
{
	u8 i;
	...
	if(Interval)//有间隔时间
	{
		...
		if(pNewRecord->TotalTim==(u32)-1) pNewRecord->TotalRem=(u32)-1;//无次数限制
		else pNewRecord->TotalRem=TimCnt-Interval;//有次数限制
...
//TimCnt为0，会执行高优先级的next func
//TimCnt为1，会执行低优先级的next func
bool AddOnceMsFunc(u32 TimCnt,pStdFunc pCallBack,int IntParam,void *pParam)
{
	return AddMsFunc(TimCnt,0,(pExpFunc)pCallBack,IntParam,pParam);
}
